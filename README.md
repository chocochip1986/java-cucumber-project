...
* All BDD code are stored under src/main/java/resources/automation/*.feature  
...
* All of cucumber scenarios are made up of steps. All steps have their corresponding step definitions.  
  All step definition files must be stored under /src/main/java/automation/step_definition/* directory  
...
* All JpaRepositories are housed under src/main/java/automation/respositories directory
#####Models
* All models or entities are housed under src/main/java/automation/models directory
...
#####Data generation
* All model data generation is housed under src/main/java/automation/data_helpers directory
#####Firing API calls
* All APIs are housed in the ApiHelper class under src/main/java/automation/api_helpers directory
#####Asserts
* All test assertations are housed in the Assert class under src/main/java/automation/asserts directory  
---
...
```
mvn test -Dcucumber.options="src/main/resources/automation/test.feature:20"
```
---
###Running via Command line with tags
Run with just 1 tag
```
mvn test -Dcucumber.options="src/main/resources/automation/test.feature:20 --tags @tagname"
```
Run with multiple tags with AND condition
```
mvn test -Dcucumber.options="src/main/resources/automation/test.feature:20 --tags @tagname1 --tags @tagname2"
```
Run with multiple tags with OR condition (Take note: No spacing between tag names)
```
mvn test -Dcucumber.options="src/main/resources/automation/test.feature:20 --tags @tagname1,@tagname2"
```
...
```
mvn test -Dcucumber.options="src/main/resources/automation/test.feature:20 --tags @tagname1" -Dspring.profiles.active=qa
```
...
```
java -cp "${$(pwd)}/deployment/uber-cdit-automation.jar:${$(pwd)}/deployment/lib/*" automation.RunCucumberTest
```
###Running via an executable jar with tags
Run with just 1 tag
```
java -cp "${$(pwd)}/deployment/uber-cdit-automation.jar:${$(pwd)}/deployment/lib/*" automation.RunCucumberTest --tags @tagname
```
Run with multiple tags with AND condition
```
java -cp "${$(pwd)}/deployment/uber-cdit-automation.jar:${$(pwd)}/deployment/lib/*" automation.RunCucumberTest --tags @tagname1 --tags ~@tagname2
```
Run with multiple tags with OR condition
```
java -cp "${$(pwd)}/deployment/uber-cdit-automation.jar:${$(pwd)}/deployment/lib/*" automation.RunCucumberTest --tags @tagname1,@tagname2
```
Run with exclude tags
```
java -cp "${$(pwd)}/deployment/uber-cdit-automation.jar:${$(pwd)}/deployment/lib/*" automation.RunCucumberTest --tags ~@tagname
